...
Audio_FileLoadCompleted_Cmd::Audio_FileLoadCompleted_Cmd(const char *const pWaveName, Wave *_pWave)
	: poWfx(nullptr),
	poRawBuff(nullptr),
	rawBuffSize(0),
	pWave(_pWave)
...
void Audio_FileLoadCompleted_Cmd::Execute() 
{
	//Debug::out("Audio_FileLoadCompleted_Cmd::Execute()\n");
...
void Audio_FileLoadCompleted_Cmd::LoadBuffer(const char *const pWaveName)
{
	assert(pWaveName);
	...
	*this->poWfx = { {0} };    // zeros out the complete structure
	...
	//Debug::out("Audio_FileLoadCompleted_Cmd::LoadBuffer(%s) end\n", pWaveName);
}