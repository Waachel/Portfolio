...
void CreateTexturePNG(const char *const pFileName)
{
	assert(pFileName);
...
void CreateTextureTGA(const char *const pFileName)
{
	assert(pFileName);
...
void gltReadTGABits(const char *pFileName, textureData &textData)
{
	...
	if(sDepth == 3)
	{
		...
		char *pNewBuff = new char[ lImageSize]();;
		...
		{		
			...
			pNewBuff[j] = pBuff[i+2];
			pNewBuff[j+1] = pBuff[i + 1];
			pNewBuff[j+2] = pBuff[i];
			pNewBuff[j+3] = 0x00;
...
void gltReadPNGBits(const char *pFileName, textureData &textData)
{
	...
	// RTFM - ED.... horrible docs... but set this to 4 forces RGBA  YEAH!
	int req_comp = 4;
	...
	case 3:    
		...
		nComponents = textureData::TEXTURE_NCOMPONENT::NCOMPONENT_RGB;
		assert(false); //<-- Direct X doesn't work in 3 byte source
		break;

	case 4:  // Most likely case
		eFormat = textureData::TEXTURE_EFORMAT::EFORMAT_BGRA;
		nComponents = textureData::TEXTURE_NCOMPONENT::NCOMPONENT_RGBA;
		break;
