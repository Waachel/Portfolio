...
namespace Azul
{
		...
		numBones(NUM_BONES)
	{
		...
		this->poSkeleton = new SkeletonMain(this->poBoneResult, numBones);
		
		assert(this->poSkeleton);

		// Get this from the AnimMan
		this->pClip = pMainClip;
		assert(pClip);
	...
	AnimTime AnimMain::FindMaxTime()
	{
		assert(pClip);
		return this->pClip->GetTotalTime();
	}

	void AnimMain::ChangeAnimType(int i)
	{
		pMainClip->ChangeAnim(i);
	}

	void AnimMain::Animate(AnimTime tCurr)
	{
		this->pClip->AnimateBones(tCurr, this->poBoneResult);
	}


	void AnimMain::PoseSkeleton(GameObjectAnimSkeleton *node)
	{
		// safety
		assert(node);
