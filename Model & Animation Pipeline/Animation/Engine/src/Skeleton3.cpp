...
namespace Azul
{
	...
	Skeleton3::Skeleton3(Bone *pBone, int _numBones)
		: pFirstBone(nullptr),
		numBones(_numBones)
	{
	...
	GameObjectAnim *Skeleton3::GetFirstBone()
	{
		assert(pFirstBone);
		return this->pFirstBone;
	}

	GameObjectAnim *Skeleton3::privFindBoneByIndex(int index)
	{
	...
	void Skeleton3::privSetAnimationHierarchy(Bone *pBoneResult)
	{
#if 1
		assert(pBoneResult);
		...
		for(int i = 0; i < 1; i++)
		{
			...
			pGameObj = new GameObjectAnimSkeleton3(pGraphicsObj, pBoneResult);
			pGameObj->SetIndex(skel[i].index);
			pGameObj->SetName(skel[i].name);
