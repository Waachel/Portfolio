...
namespace Azul
{
	...
	Skeleton2::Skeleton2(Bone *pBone, int _numBones)
		: pFirstBone(nullptr),
		numBones(_numBones)
	{
	...
	GameObjectAnim *Skeleton2::GetFirstBone()
	{
		assert(pFirstBone);
		return this->pFirstBone;
	}

	GameObjectAnim *Skeleton2::privFindBoneByIndex(int index)
	{
	...
	void Skeleton2::privSetAnimationHierarchy(Bone *pBoneResult)
	{
#if 1
		assert(pBoneResult);
		...
		for(int i = 0; i < 1; i++)
		{
			...
			pGameObj = new GameObjectAnimSkeleton2(pGraphicsObj, pBoneResult);
			pGameObj->SetIndex(skel[i].index);
			pGameObj->SetName(skel[i].name);
