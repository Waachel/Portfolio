...
namespace Azul
{
	...
	SkeletonMain::SkeletonMain(Bone *pBone, int _numBones)
		: pFirstBone(nullptr),
		numBones(_numBones)
	{
	...
	GameObjectAnim *SkeletonMain::GetFirstBone()
	{
		assert(pFirstBone);
		return this->pFirstBone;
	}

	GameObjectAnim *SkeletonMain::privFindBoneByIndex(int index)
	{
	...
	void SkeletonMain::privSetAnimationHierarchy(Bone *pBoneResult)
	{
#if 1
		assert(pBoneResult);
		...
		for(int i = 0; i < 1; i++)
		{
			...
			pGameObj = new GameObjectAnimSkeletonMain(pGraphicsObj, pBoneResult);
			pGameObj->SetIndex(skel[i].index);
			pGameObj->SetName(skel[i].name);
