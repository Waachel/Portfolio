...
namespace Azul
{
	extern Clip *pSideStepRightClip;
		...
		numBones(NUM_BONES)
	{
		...
		this->poSkeleton = new Skeleton3(this->poBoneResult, numBones);
		
		assert(this->poSkeleton);

		// Get this from the AnimMan
		this->pClip = pSideStepRightClip;
		assert(pClip);
	...
	AnimTime AnimSide::FindMaxTime()
	{
		assert(pClip);
		return this->pClip->GetTotalTime();
	}

	void AnimSide::Animate(AnimTime tCurr)
	{
		this->pClip->AnimateBones(tCurr, this->poBoneResult);
	}


	void AnimSide::PoseSkeleton(GameObjectAnimSkeleton *node)
	{
		// safety
		assert(node);
