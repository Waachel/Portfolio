...
namespace Azul
{
	...
	Skeleton4::Skeleton4(Bone *pBone, int _numBones)
		: pFirstBone(nullptr),
		numBones(_numBones)
	{
	...
	GameObjectAnim *Skeleton4::GetFirstBone()
	{
		assert(pFirstBone);
		return this->pFirstBone;
	}

	GameObjectAnim *Skeleton4::privFindBoneByIndex(int index)
	{
	...
	void Skeleton4::privSetAnimationHierarchy(Bone *pBoneResult)
	{
#if 1
		assert(pBoneResult);
		...
		for(int i = 0; i < 1; i++)
		{
			...
			pGameObj = new GameObjectAnimSkeleton4(pGraphicsObj, pBoneResult);
			pGameObj->SetIndex(skel[i].index);
			pGameObj->SetName(skel[i].name);
