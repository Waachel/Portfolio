...
namespace Azul
{
		...
		numBones(NUM_BONES)
	{
		...
		this->poSkeleton = new Skeleton1(this->poBoneResult, numBones);
		
		assert(this->poSkeleton);

		// Get this from the AnimMan
		this->pClip = pWalkClip;
		assert(pClip);
	...
	AnimTime AnimWalk::FindMaxTime()
	{
		assert(pClip);
		return this->pClip->GetTotalTime();
	}

	void AnimWalk::Animate(AnimTime tCurr)
	{
		this->pClip->AnimateBones(tCurr, this->poBoneResult);
	}


	void AnimWalk::PoseSkeleton(GameObjectAnimSkeleton *node)
	{
		// safety
		assert(node);
