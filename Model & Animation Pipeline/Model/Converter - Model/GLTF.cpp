...
bool GLTF::SetVBO(Model &gltfModel, const char *pKey, vboData &vbo, char *pBinaryBuff, int meshIndex)
{
...
bool GLTF::SetVBO_index(Model &gltfModel, const char *pKey, vboData &vbo, char *pBinaryBuff, int meshIndex)
{
	AZUL_UNUSED_VAR(pKey);

	vbo.enabled = true;

	auto index = gltfModel.meshes[meshIndex].primitives[0].indices;
//	assert(index > 0);
...
bool GLTF::SetTexture(Model &gltfModel, const char *pKey, textureData &text, char *pBinaryBuff)
{
	...
	// need a way to data drive this line of code
	text.enabled = true;

	auto index = gltfModel.materials[0].pbrMetallicRoughness.baseColorTexture.index;
	//assert(index >= 0 && index < gltfModel.images.size());
