...
{
	pHackDeviceContext->VSSetShader(poVertexShader, nullptr, 0);
	pHackDeviceContext->IASetInputLayout(poInputLayout);
	pHackDeviceContext->PSSetShader(poPixelShader, nullptr, 0);
...
const char *ShaderObject::GetLatestProfile_VertexShader() const
{
	...
	D3D_FEATURE_LEVEL featureLevel = pHackDevice->GetFeatureLevel();
									  ...
									  size_t codeSize)
{
	...
	HRESULT hr;
	hr = pHackDevice->CreateVertexShader(pCompiledVertexShaderObject, codeSize, nullptr, &poVertexShader);
	assert(SUCCEEDED(hr));
									 ...
									 size_t codeSize)
{
	...
	HRESULT hr;
	hr = pHackDevice->CreatePixelShader(pCompiledPixelShaderObject, codeSize, nullptr, &poPixelShader);
	assert(SUCCEEDED(hr));
									 ...
									 unsigned int SizeInBytes)
{
	...
	HRESULT hr;
	hr = pHackDevice->CreateInputLayout(pDesc,
