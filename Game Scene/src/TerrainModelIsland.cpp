...
TerrainModelIsland::TerrainModelIsland(ID3D11Device* dev, LPCWSTR heightmapFile, float len, float maxheight, float ytrans, int RepeatU, int RepeatV)
{
...
int TerrainModelIsland::TexelIndex(int side, int i, int j) const
{
	return pixel_width * (j * side + i);
}


TerrainModelIsland::~TerrainModelIsland()
{
	delete pModTerrain;
}

void TerrainModelIsland::Render(ID3D11DeviceContext* context)
{
	pModTerrain->SetToContext(context);
	pModTerrain->Render(context);
}
														//v2 - v1    v3 - v1
Vect TerrainModelIsland::getFaceNormal(Vect p, Vect up, Vect ul, Vect lf, Vect ld, Vect dn, Vect dr, Vect rt, Vect ru)
{
...
Matrix TerrainModelIsland::getWorldMatrix()
{
	return pWorldT;
}