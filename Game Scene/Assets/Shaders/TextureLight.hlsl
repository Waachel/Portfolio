...
float4 PS(VS_OUTPUT input) : SV_Target
{
	...
	float FogPercent = saturate((distToEye - FogParam.FogStart) / FogParam.FogRange);  // See https://msdn.microsoft.com/en-us/library/windows/desktop/bb509645(v=vs.85).aspx

	litColor = litColor * (1 - FogPercent) + FogParam.FogColor * FogPercent;

	//float4 texColor = { 1, 1, 1, 1 };
	float4 texColor = Texture.Sample(Sampler, input.Tex);
	float4 finalColor = litColor * texColor;
	return finalColor;
}
